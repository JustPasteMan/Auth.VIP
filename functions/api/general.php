...
function is_premium(mysqli_wrapper $c_con, $program_key) { //check if the user is premium with the program_key
    $program_check = $c_con->query('SELECT c_owner FROM c_programs WHERE c_program_key=?', [$program_key]);

    if ($program_check->num_rows === 0)
        return false;

    $user_check = $c_con->query('SELECT c_expires FROM c_users WHERE c_username=?', [$program_check->fetch_assoc()['c_owner']]);

    if ($user_check->num_rows === 0)
        return false;

    $timestamp = $user_check->fetch_assoc()['c_expires'];
